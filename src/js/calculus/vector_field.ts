...
export class VectorField {
  ...
  evaluateMemo: { [index: string]: Vector } = {};
  evaluate(x: number, y: number, z: number): Vector {
    ...
    let solution = new Vector(
      this.d[0].evaluate(scope),
      this.d[1].evaluate(scope),
      this.d[2].evaluate(scope)
    );
    this.evaluateMemo[key] = new Vector(
      solution.x(),
      solution.y(),
      solution.z()
    );
